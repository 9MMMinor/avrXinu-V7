...
#define IPCKOFF 10		/* IP header offset to i_cksum			*/
#define	IMAXLEN	EMAXPAK-IPHLEN-EHLEN		/* Maximum IP datagram length (Notes:	*/
...
struct	ip	{		/* Format of DARPA Internet datagram	*/
	...
	int		i_paclen;	/* packet length in octets (bytes)	*/
	int		i_id;		/* datagram id (to help gateways frag.)	*/
	int		i_fragoff;	/* fragment offset (0 for 1st fragment) */
	char	i_tim2liv;	/* time to live in gateway hops (10)	*/
	char	i_proto;	/* IP protocol (UDP is assigned 17)	*/
	int		i_cksum;	/* 1s compl. of sum of shorts in header	*/
...
#define	ITIM2LIV	64	/* Initial time-to-live value		*/

...
static int kputc(unsigned char c, FILE * stream);
static FILE kprint_out =  FDEV_SETUP_STREAM(kputc, NULLPTR, _FDEV_SETUP_WRITE);
//static void savestate(void);
//static void rststate(void);

/*  kprintf, kputc, USART_Init, USART_Transmit */
...
int kprintf(const char *fmt, ...)
{
    ...
    va_start(args, fmt);
    i = vfprintf(&kprint_out,fmt,args);
    va_end(args);

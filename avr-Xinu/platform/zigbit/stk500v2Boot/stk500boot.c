...
//* Nov 10, 2011	<MMM> = Michael Minor https://sites.google.com/site/avrxinu
//* Nov 10, 2011	<MMM> Added increased CMD_SPI_MULTI support for AVRDUDE
//*					<MMM> Note: avrdude version > 5.8 is required; avrdude-5.10 tested
//*	Sep 27,	2013	<MMM> Added support for atmega2564
//* Jan 29, 2014	<MMM> Added support for atmega256rfr2
//* Aug 15, 2014	<MMM> Basis for RF bootstrap on atmega256rfr2:
//*
//************************************************************************
...
//#if defined(_MEGA_BOARD_) || defined(_BOARD_AMBER128_) || defined(__AVR_ATmega1280__) || defined(__AVR_ATmega1284P__) || defined(__AVR_ATmega2560__) || defined(__AVR_ATmega2561__)
...
#elif defined( _MIN1284P_BOARD_ ) || defined( _AVR_P40_USB_BOARD_) || defined( _AVR_44Proto_BOARD_ ) || defined( _AVR_48Schmart_BOARD_ )
	//* 	this is for the DIY 20Mhz 1284p min1284p board
	//* 	www.kirbyand.co.uk, a sanguino upgrade
	//* 	this is for the Olimex 8Mhz (1284p) AVR-P40-USB board
	//*		this is for the DIY 11.0592 (1284p) ME-PB-SM-TQFP44-80 Proto Board
	//*		this is for DIY 16.0Mhz TQFP-48 Schmartboard (atmega2564rfr2)
	#define PROGLED_PORT	PORTB
	#define PROGLED_DDR		DDRB
	#define PROGLED_PIN		PINB0
#elif defined( _AVR_256RFR2XPLAINPRO_ )
	//*		this is for 16.0Mhz 256RFR2XplainedPro (atmega256rfr2)
	#define PROGLED_PORT	PORTB
	#define PROGLED_DDR		DDRB
	#define PROGLED_PIN		PINB4
	#define PROGBUT_PORT	PORTE
	#define PROGBUT_DDR		DDRE
	#define PROGBUT_PIN		PINE4
#elif defined( _AVR_256RFR2ZIGBIT_ )
	//*		this is for 16.0Mhz 256RFR2 Extension Board (ATZB-S1-256-3-0-C) (atmega256rfr2)
	#define PROGLED_PORT	PORTG
	#define PROGLED_DDR		DDRG
	#define PROGLED_PIN		PING2
	#define PROGBUT_PORT	PORTE
	#define PROGBUT_DDR		DDRE
	#define PROGBUT_PIN		PINE0
#else
	#define PROGLED_PORT	PORTG
	#define PROGLED_DDR		DDRG
	#define PROGLED_PIN		PING2
 ...
 */
	...
	#define	UART_DOUBLE_SPEED			U2X
	 ...
	 || defined(__AVR_ATmega1280__) || defined(__AVR_ATmega1284P__) || defined(__AVR_ATmega2560__) \
	 || defined(__AVR_ATmega2561__) || defined(__AVR_ATmega2564RFR2__) || defined(__AVR_ATmega256RFR2__)
/*
 ...
 * function prototypes
 */
static void sendchar(unsigned char c);			//// mmm 11/7/2011
static unsigned char recchar(void);
static unsigned char recchar_timeout(void);		//// mmm 11/7/2011
		...
		while (!isLeave)
		{
			...
			switch (msgBuffer[0])
			{
				...
				case CMD_SPI_MULTI:
				{
                    ...
                    // AVRDUDE needs
					// Read Signature Bytes, Read Fuses, Read Program Memory implemented
					// return 0 for other instructions
					if ( msgBuffer[4]== 0x30 )
					{
						answerByte = boot_signature_byte_get(msgBuffer[6]<<1);
					}
					else if (msgBuffer[4] == 0x50 && msgBuffer[5] == 0x00)
					{
						answerByte = boot_lock_fuse_bits_get(GET_LOW_FUSE_BITS);
					}
					else if (msgBuffer[4] == 0x58 && msgBuffer[5] == 0x08)
					{
						answerByte = boot_lock_fuse_bits_get(GET_HIGH_FUSE_BITS);
					}
					else if (msgBuffer[4] == 0x50 && msgBuffer[5] == 0x08)
					{
						answerByte = boot_lock_fuse_bits_get(GET_EXTENDED_FUSE_BITS);
					}
					else if (msgBuffer[4] == 0x20)	// Read Program Memory, Low Byte
					{
						address = ( ((msgBuffer[5])<<8)|(msgBuffer[6]) )<<1;  //convert word to byte address
						answerByte = pgm_read_byte_near(address);
					}
					else if (msgBuffer[4] == 0x28)	// Read Program Memory, High Byte
					{
						address = ( ((msgBuffer[5])<<8)|(msgBuffer[6]) )<<1;  //convert word to byte address
						answerByte = pgm_read_byte_near(address+1);
					}
    				msgLength = 7;
...
#elif defined(__AVR_ATmega2564rfr2__)
	#define	kCPU_NAME	"ATmega2564rfr2"
#elif defined(__AVR_ATmega256rfr2__)
	#define	kCPU_NAME	"ATmega256rfr2"
#endif
